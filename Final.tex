...
\begin{figure}[htbp]
 \centerline{ \includegraphics[scale=0.90]{multi.jpg}}
...
\begin{figure}[htbp]
 \centerline{ \includegraphics[scale=0.70]{star.jpg}}
...
\begin{figure}[htbp]
 \centerline{ \includegraphics[scale=0.70]{hybride.jpg}}
...
\begin{figure}[htbp]
\includegraphics[scale=0.5]{Cycle.jpg}
...
FlexRay, compared to CAN is able to communicate multiple times faster, which makes it a more "flexible" message transferring system. As it is developed to be used in the automative field, finding any other application outside the automative section, is not likely. Despite the fact that FleyRay has not much acceptance in volume vehicles, it is the most advanced technology used in high class vehicles. The reasons for this can be explained via the risks of a new technology, higher costs and the training of the personnel. However, with the rising data volumes, which needs to be processed, are demanding for even higher bandwidths \cite{hammerschmidt2010beyond}. Developments such as real-time image or video data generated by multiple cameras are practiced for computing a digital representation of the surrounding space, in order to detect obstacles or recognize pedestrians. These technologies are already implemented by some manufacturers, such as Tesla. Therefore to be able to implements faster communications with large amounts of data, another technology must be used. As a result a faster system named Ethernet/IP was developed. And because of demands, such as Advanced Driver-Assistance Systems (ADAS) or other In-car entertainment features Ethernet is expecting a faster increase of use in cat networks. Nevertheless, FlexRay can also have greater adoption if there are for instance reduced costs or more X-by-Wire cases.\\


\section*{CONTRIBUTION AND TEAM WORKING}

\begin{table}[h!]
\caption{Writing Responsabilities}
\begin{tabular}{|l|l|}
\hline
Tuna & \begin{tabular}[c]{@{}l@{}}Abstract, Introduction, History Of FlexRay, \\ Communication Cycle, Comparison With Other Protocols, \\ FlexRay Applications, Conclusion\end{tabular} \\ \hline
Bala & Characteristics Of FlexRay, Network Topology                                                                                                                                      \\ \hline
\end{tabular}
\end{table}
